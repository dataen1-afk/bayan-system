...
class BilingualContractPDFGenerator:
    ...
    def generate_bilingual_contract(self, agreement_data: dict, proposal_data: dict) -> bytes:
        ...
        if proposal_data.get('notes'):
            story.append(Spacer(1, 6))
            story.append(Paragraph(f"<b>{self.TRANSLATIONS['notes']['en']} | {process_arabic_text(self.TRANSLATIONS['notes']['ar'])}:</b> {proposal_data.get('notes')}", self.styles['TextEN']))
        ...
        sig_data = [
            ...
            [f"{self.TRANSLATIONS['authorized_signatory']['en']}", 
             agreement_data.get('signatory_name', '')],
