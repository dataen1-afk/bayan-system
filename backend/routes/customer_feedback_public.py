...
async def get_public_feedback_form(access_token: str):
    ...
    feedback = await doc_pg.get_by_payload_field(
        doc_pg.C_CUSTOMER_FEEDBACK, "access_token", access_token
    )
...
async def submit_public_feedback(access_token: str, data: CustomerFeedbackSubmit):
    ...
    await doc_pg.replace_payload(doc_pg.C_CUSTOMER_FEEDBACK, fid, merged)
