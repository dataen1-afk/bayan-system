...
# NOTE: Technical Review Models (BACF6-15) moved to routes/technical_reviews.py

# NOTE: Customer Feedback Models (BACF6-16) moved to routes/customer_feedback.py

# NOTE: Pre-Transfer Review Models (BACF6-17) moved to routes/pre_transfer_reviews.py

# NOTE: Certified Clients Models (BAC-F6-19) moved to routes/certified_clients.py

# NOTE: Suspended Clients Models (BAC-F6-20) moved to routes/suspended_clients.py
...
async def send_form_email(form_id: str, current_user: dict = Depends(require_admin)):
    ...
    frontend_url = os.environ.get('FRONTEND_URL', 'https://contract-audit-flow.preview.emergentagent.com')
...
# NOTE: RFQ & Contact Form Endpoints moved to routes/portal.py
...
# NOTE: Technical Review Endpoints (BACF6-15) moved to routes/technical_reviews.py

# NOTE: Customer Feedback Endpoints (BACF6-16) moved to routes/customer_feedback.py

# NOTE: Pre-Transfer Review Endpoints (BACF6-17) moved to routes/pre_transfer_reviews.py

# NOTE: Certified Clients Endpoints (BAC-F6-19) moved to routes/certified_clients.py

# NOTE: Suspended Clients Endpoints (BAC-F6-20) moved to routes/suspended_clients.py
...
async def create_certificate(cert_data: CertificateCreate, credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    base_url = os.environ.get('FRONTEND_URL', 'https://contract-audit-flow.preview.emergentagent.com')
...
async def clear_all_test_data(current_user: dict = Depends(require_admin)):
    """
    Delete all test/experimental data from the database.
    Only accessible by system_admin users.
    Preserves user accounts.
    """
    # Only allow system_admin to perform this action
    if current_user.get('role') != 'system_admin':
        raise HTTPException(status_code=403, detail="Only system administrators can perform this action")
    
    # Collections to clear (preserving users)
    collections_to_clear = [
        'application_forms',
        'proposals', 
        'contracts',
        'certification_agreements',
        'contract_reviews',
        'audit_programs',
        'job_orders',
        'stage1_audit_plans',
...
async def get_data_summary(current_user: dict = Depends(require_admin)):
    """Get a summary of all data in the system for review before deletion."""
    
    collections_to_check = [
        ('application_forms', 'طلبات المنح'),
        ('proposals', 'عروض الأسعار'),
        ('contracts', 'العقود'),
        ('certification_agreements', 'اتفاقيات الاعتماد'),
        ('contract_reviews', 'مراجعات العقود'),
        ('audit_programs', 'برامج التدقيق'),
