...
export default function PublicStage1PlanReviewPage() {
  ...
  const fetchPlan = async () => {
    try {
      const response = await axios.get(`${API_URL}/api/public/stage1-audit-plans/${accessToken}`);
  ...
  const handleAccept = async () => {
    setSubmitting(true);
    try {
      await axios.post(`${API_URL}/api/public/stage1-audit-plans/${accessToken}/respond`, {
        accepted: true,
        change_requests: ''
      });
      setSubmitted(true);
    ...
    <div className={`min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 py-8 px-4 ${isRTL ? 'rtl' : 'ltr'}`} dir={isRTL ? 'rtl' : 'ltr'}>
      <div className="max-w-3xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-900">{t('stage1AuditPlanReview') || 'Stage 1 Audit Plan Review'}</h1>
          <p className="text-gray-500">BACF6-07</p>
        </div>
        ...
        <div className="text-center text-sm text-gray-500">
          ...
          <p className="mt-1">{t('stage1Footer') || 'Stage 1 Audit Plan - BACF6-07'}</p>
        </div>
      </div>
    </div>
  );
