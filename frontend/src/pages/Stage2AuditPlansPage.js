...
export default function Stage2AuditPlansPage({ embedded = false }) {
  ...
  const [createSource, setCreateSource] = useState('stage1'); // 'stage1' or 'jobOrder'
  ...
  const fetchData = async () => {
    try {
      ...
      const [plansRes, stage1Res, ordersRes, auditorsRes] = await Promise.all([
      ...
      // Only Stage 1 plans that are client_accepted
      setStage1Plans(stage1Res.data.filter(p => p.status === 'client_accepted'));
  ...
  const createPlan = async () => {
    const payload = { audit_type: auditType };
    
    if (createSource === 'stage1' && selectedStage1Id) {
      payload.stage1_plan_id = selectedStage1Id;
    } else if (createSource === 'jobOrder' && selectedJobOrderId) {
      payload.job_order_id = selectedJobOrderId;
    } else {
      alert(t('selectSource') || 'Please select a source');
      return;
    }
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post(
        `${API_URL}/api/stage2-audit-plans`,
        payload,
  ...
  const updatePlan = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(
        `${API_URL}/api/stage2-audit-plans/${selectedPlan.id}`,
        editForm,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const deletePlan = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`${API_URL}/api/stage2-audit-plans/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchData();
    } catch (error) {
  ...
  const managerApprove = async (planId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(
        `${API_URL}/api/stage2-audit-plans/${planId}/manager-approve`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      fetchData();
  ...
  const sendToClient = async (plan) => {
    try {
      ...
      const response = await axios.post(
        `${API_URL}/api/stage2-audit-plans/${plan.id}/send-to-client`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const downloadPDF = async (plan) => {
    try {
      ...
      const response = await axios.get(
        `${API_URL}/api/stage2-audit-plans/${plan.id}/pdf`,
        { 
          headers: { Authorization: `Bearer ${token}` },
          responseType: 'blob'
        }
      ...
      link.href = url;
      link.setAttribute('download', `stage2_audit_plan_${plan.organization_name.replace(/\s+/g, '_')}.pdf`);
  ...
  const openEditModal = (plan) => {
    ...
    setEditForm({
      ...
      audit_type: plan.audit_type || 'Stage 2',
  ...
  // Get available Stage 1 plans that don't have a Stage 2 yet
  const availableStage1Plans = stage1Plans.filter(
    p => !plans.some(s2 => s2.stage1_plan_id === p.id)
  );
  
  // Get confirmed job orders
  const availableJobOrders = jobOrders.filter(
    o => !plans.some(p => p.job_order_id === o.id && !p.stage1_plan_id)
  );
    ...
    <div className={`${embedded ? '' : 'p-6'} ${isRTL ? 'rtl' : 'ltr'}`} dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      {!embedded && (
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">{t('stage2AuditPlans') || 'Stage 2 Audit Plans'}</h1>
            <p className="text-gray-500">BACF6-08 - {t('stage2Description') || 'Certification audit - Phase 2 planning'}</p>
          </div>
          <Button onClick={() => setShowCreateModal(true)} className="bg-violet-600 hover:bg-violet-700" data-testid="create-stage2-plan-btn">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            {t('stage2AuditPlans') || 'Stage 2 Audit Plans'}
          </CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-8 text-gray-500">
              ...
              <p>{t('noStage2Plans') || 'No Stage 2 Audit Plans yet'}</p>
              <p className="text-sm mt-2">{t('createFromStage1') || 'Create a plan from a Stage 1 plan or job order'}</p>
            </div>
          ) : (
              ...
              <table className="w-full table-fixed">
                <thead>
                  <tr className={`border-b ${isRTL ? 'text-right' : 'text-left'}`}>
                    <th className={`p-3 px-4 font-medium text-gray-600 w-[180px] ${isRTL ? 'text-right' : 'text-left'}`}>{t('organization') || 'Organization'}</th>
                    <th className={`p-3 px-4 font-medium text-gray-600 w-[100px] ${isRTL ? 'text-right' : 'text-left'}`}>{t('auditType') || 'Audit Type'}</th>
                    <th className={`p-3 px-4 font-medium text-gray-600 w-[150px] ${isRTL ? 'text-right' : 'text-left'}`}>{t('teamLeader') || 'Team Leader'}</th>
                ...
                <tbody>
                  {plans.map(plan => (
                    <tr key={plan.id} className="border-b hover:bg-gray-50" data-testid={`stage2-plan-row-${plan.id}`}>
                      ...
                      <td className={`p-3 px-4 ${isRTL ? 'text-right' : ''}`}>
                        <span className="px-2 py-1 bg-violet-100 text-violet-800 rounded text-sm whitespace-nowrap">
                          {plan.audit_type || 'Stage 2'}
                        </span>
                      </td>
                      <td className="p-3 px-4">
                        <div className={`flex items-center gap-2 ${isRTL ? 'flex-row-reverse justify-end' : ''}`}>
      ...
      <Dialog open={showCreateModal} onOpenChange={setShowCreateModal}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>{t('createStage2Plan') || 'Create Stage 2 Audit Plan'}</DialogTitle>
            <DialogDescription>{t('selectCreateSource') || 'Select where to create from'}</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            {/* Source selection */}
            <div>
              <Label>{t('createFrom') || 'Create From'}</Label>
              <div className="flex gap-4 mt-2">
                <Button 
                  variant={createSource === 'stage1' ? 'default' : 'outline'}
                  onClick={() => setCreateSource('stage1')}
                  className={createSource === 'stage1' ? 'bg-violet-600' : ''}
                >
                  {t('fromStage1') || 'From Stage 1 Plan'}
                </Button>
                <Button 
                  variant={createSource === 'jobOrder' ? 'default' : 'outline'}
                  onClick={() => setCreateSource('jobOrder')}
                  className={createSource === 'jobOrder' ? 'bg-violet-600' : ''}
                >
                  {t('fromJobOrder') || 'From Job Order'}
                </Button>
              </div>
            </div>
            ...
            <div className="flex gap-2 justify-end">
              ...
              <Button 
                onClick={createPlan} 
                disabled={(createSource === 'stage1' && !selectedStage1Id) || (createSource === 'jobOrder' && !selectedJobOrderId)}
      ...
      <Dialog open={showEditModal} onOpenChange={setShowEditModal}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{t('stage2PlanDetails') || 'Stage 2 Audit Plan Details'}</DialogTitle>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-4 gap-4">
                  <div>
                    <Label>{t('auditType') || 'Audit Type'}</Label>
                    <Select value={editForm.audit_type} onValueChange={(v) => setEditForm({...editForm, audit_type: v})}>
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
              ...
              <div className="flex gap-2 justify-end">
                ...
                <Button onClick={updatePlan} data-testid="save-plan-btn" className="bg-violet-600 hover:bg-violet-700">
